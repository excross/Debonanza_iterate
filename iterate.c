...
// iterate.c 反復深化
/* value = searchr( ptree, root_alpha, root_beta, root_turn, rand_depth * PLY_INC); */
...
iterate( tree_t * restrict ptree )
{
  ...
  /* probe the opening book */ /* Opening_book を検索する. */
  if ( pf_book != NULL
       ...
       && ! rep_book_prob( ptree ) )
    {
      ...
      iret = get_elapsed( &elapsed ); /* 経過の取得 */
  ...
  /* 稲庭戦術-検出 */ /* 稲庭戦術:  歩を一切伸ばさず、全ての歩に2つ以上の利きを持たせる.時間切れ勝ち狙い. */
    /* 詳細URL: http://blogs.yahoo.co.jp/ten_nan_91/14128172.html */
...

  {
    ...
    /* 手数に応じて、time_limitとtime_max_limitを割り当てる。基本的に前者の5倍が後者。 */
  ...
  /* probe the transposition table, since last pv is not available.  */
    /* Last PV が使用できなくなるため、 Transposition テーブルを調べる. */
  if ( ! last_pv.length
#if defined(MPV)
       && ! root_mpv
#endif
...
#if defined(DFPN_CLIENT)
  /* probe results from DFPN searver */ /* DFPN サーバから結果を調べる. */
  ...
  /* save preliminary result */ /* 予備の結果を保存 */
...
#if defined(DFPN_CLIENT)
  /* send the best move to DFPN server */ /* DFPN サーバに最善手を送信 */
  ...
  /* send the best move to parallel server */ /* Parallel(平行)サーバに最善手を送信 */
  if ( sckt_mnj != SCKT_NULL ) {

    if ( moves_ignore[0] == MOVE_NA && root_nmove == 1 ) {
    /* only one replay */ /* 唯一のリプレイ */
  ...
  /* return, if previous pv is long enough */ /* 前回のPVが十分な長さならば、Return */
  ...
  /* iterative deepening search */ /* 反復深化探索 */
#if defined(TLP)
  iret = tlp_start();
  if ( iret < 0 ) { return iret; }
#endif
  ...
  for ( ; iteration_depth < 30/*PLY_MAX-10*/; iteration_depth++ ) {
    ...
    /* a trial of searches */ /* 検索の試験 */
      /*  go -> searchr */
    for ( ;; ) {
        int correction_depth; // 探索を延長する量
        int d_inclination; // 探索延長の減少する傾き
        int max_correction_depth = 5; // 探索の最大の延長量
        int u_boarder_range = -600; // 不利であり、延長を行う必要のない境界線
        int t_boarder_range = 400; // 有利であり、延長を行う必要のない境界線
        int u_critical_range = -200; // 重要局面の範囲の下の値
        int t_critical_range = 0; // 重要局面の範囲の上の値
        int eval_point = evaluate( ptree, ply, root_turn ); // 現在の評価値
        if(eval_point <= u_boarder_range){ // ①延長領域外
        }else if(eval_point < u_critical_range){ // ②重要な局面ではなく、不利な局面である
            d_inclination = -1*max_correction_depth/((u_critical_range-u_boarder_range)*(u_critical_range-u_boarder_range))
            correction_depth = d_inclination * (eval_point-u_critical_range) * (eval_point-u_critical_range) + max_correction_depth;
        }else if(eval_point <= 0){ // ③重要な局面
            correction_depth = max_correction_depth;
        }else if(eval_point <= t_boarder_range){ // ④重要な局面ではなく、有利な局面
            d_inclination = -1*max_correction_depth/((t_boarder_range-t_critical_range)*(t_boarder_line-t_critical_range))
            correction_depth = d_inclination * (eval_point-t_critical_range) * (eval_point-t_critical_range) + max_correction_depth;
        }else{ // t_boarder_line < eval_point ⑤延長領域外
        }
        value = searchr( ptree, root_alpha, root_beta, root_turn, iteration_depth*PLY_INC + correction_depth*PLY_INC);
        /* iteration_depth*PLY_INC + correction_depth*PLY_INC -> 通常の探索の深さ + 延長した探索の深さ */
    ...
    /* the trial of search is finished */ /* 検索の試験の終了 */
  ...
  /* prunings and extentions-statistics */ /* 剪定 & 拡張統計 */
  {
    double drep, dhash, dnull, dfh1st;
    /* 予測 rep -> repeat */
    drep    = (double)ptree->nperpetual_check; /* 非永久的なチェック. */
    drep   += (double)ptree->nfour_fold_rep; /* 4倍繰り返していない. */ 
    drep   += (double)ptree->nsuperior_rep; /* 優れていない繰り返し. */
    drep   *= 100.0 / (double)( ptree->nrep_tried + 1 ); /* 繰り返さない実行. */

    dhash   = (double)ptree->ntrans_exact; /* 遷移なし正確 */
    dhash  += (double)ptree->ntrans_inferior_hit; /* trans なし、劣った発見. */
    ...
    /* 
     * check_extension_done: 王手拡張-実行済み
     * recap_extension_done: 取り返し拡張-実行済み
     * onerp_extension_done: OneReplay拡張-実行済み
     */
     
    /*
     * Out: 呼び出し先のプロシージャから呼び出し元のプロシージャから返された
     *      ポインターのパラメーターを識別します (サーバーからクライアントに)。
     */
